...
## Generating Random Search Maps

A primitive implementation exists that generates random search areas of size MxN. Parameters for this generation are found in `config/gen_search_map.yaml`. The search maps are generated by stacking various gaussians. Then, a weighted sample on this map is used to generate target positions. To generate the search maps, run the following commands

```
roscd metrics/scripts
python3 gen_rand_search_map.py
```
