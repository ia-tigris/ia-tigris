...
class ParameterSweepManager(object):
    def __init__(self, folder_path, sigmas_list):
    ...
    def run_parameter_sweep(self):
        ...
        #run tests
        for sigma_param in self.sigmas_list:
            #create folder for this set of parameters
            folder_name = "sigmas_" + str(sigma_param)
            ...
            self.launch_args = ['metrics', 'sim_mc_runs.launch','rviz:=false', 'include_cpu_mem_monitor:=true', 
                ...
                'search:=true', 'track:=false', 'mc_config:=mc_testing_search1_random_q3.yaml', 
            ...
            rospy.logwarn("Parameter set to: sigma_naum: {}".format(sigma_param))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        sigmas_list = sys.argv[2].split(',')
        sigmas_list = [eval(i) for i in sigmas_list]
    else:
        folder_path = "/home/moon/code/onr_ws/src/ipp_planners/test_results/param_sweep_execute/"

    manager = ParameterSweepManager(folder_path, sigmas_list)
