...
RUN echo "Installing planner_ws dependencies... \n" &&\
    ...
    sudo apt-get install --no-install-recommends ros-melodic-geographic-msgs ros-melodic-geometry -y &&\
    ...
    sudo apt-get install ros-melodic-gazebo-msgs --no-install-recommends &&\
...
# FROM ros:melodic-ros-base as ipp_planner_dev
