...
static void BM_SearchMap_Search_Information_Gain_Hashmap(benchmark::State& state) 
{
    ...
    node->information = map.search_information_gain_with_hashmaps(node, state.range(0), desired_speed);
    ...
    node2->parent = node;
    node2->information = map.search_information_gain_with_hashmaps(node2, state.range(0), desired_speed);
    ...
    node3->parent = node2;
    node3->information = map.search_information_gain_with_hashmaps(node3, state.range(0), desired_speed);
    ...
    node4->parent = node3;
    node4->information = map.search_information_gain_with_hashmaps(node4, state.range(0), desired_speed);
    ...
    node5->parent = node4;
    node5->information = map.search_information_gain_with_hashmaps(node5, state.range(0), desired_speed);
    ...
    node6->parent = node5;
    node6->information = map.search_information_gain_with_hashmaps(node6, state.range(0), desired_speed);
    ...
    node7->parent = node6;
    node7->information = map.search_information_gain_with_hashmaps(node7, state.range(0), desired_speed);
    
    for (auto _ : state)
    {
        benchmark::DoNotOptimize(map.search_information_gain_with_hashmaps(node7, state.range(0), desired_speed));
        benchmark::ClobberMemory();
    }     
}
BENCHMARK(BM_SearchMap_Search_Information_Gain_Hashmap)->Args({true, 10})->Args({false, 10})->Args({true, 8})->Args({false, 8});

static void BM_SearchMap_Estimtate_Belief_And_Reward_Hashmap(benchmark::State& state) 
{
    ompl::base::SpaceInformationPtr si_xyzpsi = GetStandardXYZPsiSpacePtr();
    std::vector<std::vector<double>> polygon_bounds;
    polygon_bounds = {{-495, -495}, {-495, 49900}, {49900, 49900}, {45900, 25900}, {49900, -495}};
    ...
    for (auto _ : state)
    {
        benchmark::DoNotOptimize(map.estimate_belief_and_reward_with_hashmap(node, projected_camera_bounds, current_node, bbox));
        benchmark::ClobberMemory();
    }     
}
BENCHMARK(BM_SearchMap_Estimtate_Belief_And_Reward_Hashmap)->Args({10})->Args({8});
