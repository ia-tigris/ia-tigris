...
namespace ipp
{
    class PrimTreeBnB : public Planner
    {
    protected:
        TreeNode *tree_root;
        ...
        double curr_best_info_gain;
        double sensor_projection_width;
    public:
        double primtreebnb_extend_radius;
        double primtreebnb_extend_angle;
        double primtreebnb_extend_dist;
        double num_cell_upperbound;
        std::string max_mode;

        int primtreebnb_branches_each_side;
        std::string planner_name = "PrimTreeBnB";

        PrimTreeBnB(ros::NodeHandle &nh, ros::NodeHandle &pnh): 
            Planner(nh, pnh),
            primtreebnb_extend_radius(ros_utils::get_param<double>(pnh, "primtreebnb_extend_radius")),
            primtreebnb_extend_angle(ros_utils::get_param<double>(pnh, "primtreebnb_extend_angle")),
            primtreebnb_extend_dist(ros_utils::get_param<double>(pnh, "primtreebnb_extend_dist")),
            num_cell_upperbound(ros_utils::get_param<double>(pnh, "num_cell_upperbound")),
            max_mode(ros_utils::get_param<std::string>(pnh, "max_mode")),
            primtreebnb_branches_each_side(ros_utils::get_param<double>(pnh, "primtreebnb_branches_each_side"))
        {
            ...
            // get the width of camera FOV width
            std::vector<std::vector<double>> q_rotated = rotated_camera_fov(sensor_params, /*roll*/ 0.0, /*sensor_params.pitch*/ sensor_params.pitch, /*yaw*/ 0.0);
            std::vector<double> node_pos = {0.0, 0.0, flight_height};
            std::vector<std::vector<double>> static_camera_bounds = ::project_camera_bounds_to_plane(node_pos, q_rotated, sensor_params.highest_max_range);
            double diff_x = static_camera_bounds[2][0] - static_camera_bounds[1][0];
            double diff_y = static_camera_bounds[2][1] - static_camera_bounds[1][1];
            sensor_projection_width = sqrt(diff_x * diff_x + diff_y * diff_y);
            ROS_WARN_STREAM("width of the projected sensor FOV is " << sensor_projection_width);
        }
        ...
        bool replan_setup(InfoMap &info_map, std::vector<double> start_pose, double budget, bool force_from_scratch)
        {
            ...
            this->update_tree(start_pose, info_map, true);
        ...
        bool replan_loop(InfoMap &info_map, std::vector<double> start_pose, double budget, bool should_clear_tree)
        {
            ...
            nodes_to_expand.pop();
            if (should_expand_branch(info_map, node_to_add))
            {
        ...
        bool add_node_to_tree(InfoMap &info_map, TreeNode *add_node, TreeNode *add_node_parent)
        {
            if (add_node != nullptr)
            {
                ...
                if (add_node->information > this->leaf_node_of_best_path->information)
                {
                    ...
                    // update this value for comparison in branch and bound
                    curr_best_info_gain = this->leaf_node_of_best_path->information;
                }
            }
            return true;
        }
        ...
        bool should_expand_branch(InfoMap &info_map, TreeNode* node)
        {
            double remaining_distance = node->budget_remaining;
            const SearchMap* map = info_map.get_search_map();

            if (sensor_projection_width <= 0)
            {
                ROS_WARN_STREAM("sensor projection width < 0, something wrong may happen");
            }
            
            double size_fov_area;
            double remaining_cover_area = sensor_projection_width * remaining_distance + size_fov_area;
            double remaining_cover_cells = remaining_cover_area / (double)(map->map_resolution * map->map_resolution);
            double future_gain_upper_bound;
            
            // ROS_WARN_STREAM("sensor projection width is " << sensor_projection_width);
            // ROS_WARN_STREAM("remaining budget is " << remaining_distance);
            // ROS_WARN_STREAM("remaining_cover_area " << remaining_cover_area);
            // ROS_WARN_STREAM("map cell size is " << map->map_resolution);
            // ROS_WARN_STREAM("remaining_cover_cells " << remaining_cover_cells);
            double best_info_so_far = this->leaf_node_of_best_path->information;

            // ROS_WARN_STREAM("max mode is " << max_mode);

            if (max_mode == "max_one_in_map")
            {
                double remain_info_upper_bound = node->information + remaining_cover_cells * get_max_value(map->map);
                // ROS_WARN_STREAM("sensor projection width is " << sensor_projection_width);
                // ROS_WARN_STREAM("remaining budget is " << remaining_distance);
                // ROS_WARN_STREAM("remaining_cover_area " << remaining_cover_area);
                // ROS_WARN_STREAM("map cell size is " << map->map_resolution);
                ...
                // ROS_WARN_STREAM("curent node info is " << node->information);
                // ROS_WARN_STREAM("max_possible info in map is " << get_max_value(map->map));
                // ROS_WARN_STREAM("remain_info_upper_bound is " << remain_info_upper_bound);

                // ROS_WARN_STREAM("best_branch_info is " << best_info_so_far << "\n");

                if (best_info_so_far > remain_info_upper_bound)
                {
                    return false;
                }
                else
            ...
            else if (max_mode == "max_n_in_map")
            {
                int num_cells_care;
                if (num_cell_upperbound > get_map_size(map->map))
                {
                    num_cells_care = get_map_size(map->map);
                    ROS_WARN_STREAM("N cells is larget than the map size, can only take the map size for info upperbound estimation");
                }
                else
                {
                    num_cells_care = num_cell_upperbound;
                }

                double remain_info_upper_bound;

                if (num_cells_care >= remaining_cover_cells)
                {
                    std::vector<double> remain_info_list = getTopNValues(map->map, remaining_cover_cells);
                    remain_info_upper_bound = node->information + sum_vector(remain_info_list);
                }
                else
                {
                    std::vector<double> remain_info_list = getTopNValues(map->map, num_cells_care);
                    double min_in_list = 0;
                    if (!remain_info_list.empty())
                    {
                        min_in_list = remain_info_list.front();
                    }
                    remain_info_upper_bound = node->information + sum_vector(remain_info_list) + (remaining_cover_cells - num_cells_care) * min_in_list;
                }
                ...
                // ROS_WARN_STREAM("num_cells_care is " << num_cells_care);
                // ROS_WARN_STREAM("remaining_cover_cells is " << remaining_cover_cells);
                // ROS_WARN_STREAM("remain_info_upper_bound is " << remain_info_upper_bound);
            ...
            // maximum possible, that is 1 for each grid
            else
            {
        ...
        int get_map_size(const std::vector<std::vector<double>>& map)
        {
            int totalSize = 0;
        ...
        double get_max_value(const std::vector<std::vector<double>>& map) 
        {
            double maxValue = std::numeric_limits<double>::lowest(); // Initialize with the lowest possible value
            ...
            for (const auto& row : map) 
            {
                for (double value : row) 
                {
                    if (value > maxValue) 
                    {
                        maxValue = value; // Update maxValue if a new maximum is found
                    }
                }
            }
            return maxValue;
        ...
        std::vector<double> getTopNValues(const std::vector<std::vector<double>>& map, int N) 
        {
            // Using a min-heap to keep track of top N values
            ...
            for (const auto& row : map) 
            {
                for (double value : row) 
                {
                    if (minHeap.size() < N) 
                    {
                        minHeap.push(value); // If the heap has less than N elements, add the new value
                    } else if (value > minHeap.top()) {
                        minHeap.pop(); // Remove the smallest element
                        minHeap.push(value); // Insert the new value
                    }
                }
            }
            
            // Extract the elements from the min-heap into a vector
            std::vector<double> topNValues;
            while (!minHeap.empty()) {
                topNValues.push_back(minHeap.top());
                minHeap.pop();
            }
            
            // The elements in topNValues are in ascending order, you might want to reverse it to get descending order
            // std::reverse(topNValues.begin(), topNValues.end());
        ...
        void expand_children_from_node(InfoMap &info_map, TreeNode* parent_node)
        {
            ...
            TreeNode* str_child = get_child_node(info_map, parent_node, turning_psi, primtreebnb_extend_radius);
            nodes_to_expand.push(str_child);

            double step_angle = primtreebnb_extend_angle / (2 * (double)primtreebnb_branches_each_side);
            for (int i = 0; i < primtreebnb_branches_each_side; i++)
            {
                // left
                double left_psi = step_angle * (double)(i + 1);
                TreeNode* left_child = get_child_node(info_map, parent_node, left_psi, primtreebnb_extend_radius);
                nodes_to_expand.push(left_child);

                // right
                double right_psi = - step_angle * (double)(i + 1);
                TreeNode* right_child = get_child_node(info_map, parent_node, right_psi, primtreebnb_extend_radius);
        ...
        TreeNode* get_child_node(InfoMap &info_map, TreeNode* parent_node, double turning_psi, double radius)
        {
            ...
            double child_x = start_pos->getX() + primtreebnb_extend_radius * cos(child_psi);
            double child_y = start_pos->getY() + primtreebnb_extend_radius * sin(child_psi);
            ...
            TreeNode *feasible_child_node = this->extend_from_start_node_toward_goal_node(info_map, parent_node, child_node, this->primtreebnb_extend_dist);
